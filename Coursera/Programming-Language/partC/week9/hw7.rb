...
class GeometryValue 
  ...
  #LineSegment调用其他value时，是调用以下这个方法。
  #其他value，比如Point,只要完善自己的intersecWithSegmentAsLineResult方法就行了。
  #具体来说，下面这个方法会讲LineSegment延伸成线，然后调用自己的intersect方法去判断自己和这条线是否相交
  #如果不相交，那么返回的肯定是NoPoint了。
  #如果相交，那么只要调用intersectWithSedmentAsLineResult进行进一步判断就行了。
...
class Point < GeometryValue
  ...
  #会先调用父类的intersectLineSegment方法，如果是不是NoPoints,才会调用下面的方法。
  #所以下面的方法的预设逻辑就是seg延伸的线和自己是相交的。
  def intersectWithSegmentAsLineResult seg
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    ...
    #1.00000999>1.0然而小于epsilon，所以不能这么写
  ...
  #所以下面的方法的预设逻辑就是seg延伸的线和自己是相交的。
  def intersectWithSegmentAsLineResult seg
    #seg可能是有斜率的线，也有可能是verticalline，所以需要分情况讨论.
    #verticalLIne
    seg1 = [@x1, @y1, @x2, @y2]
    seg2 = [seg.x1, seg.y1, seg.x2, seg.y2]
    if real_close(@x1, @x2)
      aXstart, aYstart, aXend, aYend, bXstart, bYstart, bXend, bYend = @y1 < seg.y1 ? seg1+seg2 : seg2+seg1
      ...
      if aXend < bXstart
        NoPoints.new
      elsif real_close(aXend, bXstart)
